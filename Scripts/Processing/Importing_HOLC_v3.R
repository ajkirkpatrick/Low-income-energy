...
## Get state for HOLC ##
HOLC[,c('STATE_ABBR','STATE_FIPS')] = US[sapply(st_intersects(HOLC, US), '[', 1), c('STUSPS','STATEFP'), drop=T] #--> ahh, drop=T here drops the sfc/geom column.
...
# HOLC.d = read_csv(file=file.path(DATA.IN, 'HOLC Maps','holc_ad_data09252017.csv')) %>%
#             arrange(polygon_id, cat_id, sub_cat_id, `_order`) %>%
#             dplyr::rename(order = `_order`)

HOLC.d = read_csv(file=file.path(DATA.IN, 'HOLC Maps','holc_ad_data04172018.csv')) %>%
...
HOLC.dd = jExtract.HOLC(HOLC.d[!is.na(HOLC.d$form_category),]) #--> 363 records (in 4-17-2018 data) have ONLY 1 entry, a text string describing the area. These return NA 
...
#   template = edit(template)
  ...
  # "few" = 1%
  # "scattered" = 2-5%
# write.csv(template, file=file.path(WORK.OUT, 'Black_Presence_v1_mapping.csv'), row.names=F) # orig in September 27, 2017; copied to 4-18 in Low Income Energy
template = read_csv(file.path(BASE, "ajk41/Low Income Energy/Data/OUTPUT/2018-04-18/Black_Presence_v1_mapping.csv"), na = c("NA")) %>%
  dplyr::mutate(NBlack_PCT = ifelse(NBlack_PCT=='I', -999, NBlack_PCT)) %>%
  dplyr::mutate(NBlack_PCT = as.numeric(NBlack_PCT)) # - yes, some are .1% and such.
...
HOLC = merge(x=HOLC, y=HOLC.dd[,c('polygon_id','NBlack_YN','NBlack_PCT','Rent35_Mean','Rent3739_Mean','Minc','repair_class','ExtraData')], by='polygon_id', 
             all.x=T, all.y=F) %>% replace_na(list(ExtraData = F))
...
xx = HOLC %>% dplyr::filter(map_id==69) #--> 68 is MD, which has lots of info digitized.
...
HOLC = HOLC %>% mutate(COUNTYFP = FIPS[unlist(st_intersects(st_centroid(HOLC), FIPS, sparse=T)),'COUNTYFP'] %>% pull(COUNTYFP))
HOLC = HOLC %>% mutate(STCOFIPS = paste0(STATE_FIPS, COUNTYFP))
...
using.fips = unique(HOLC[,c('STATE_FIPS','COUNTYFP')] %>% st_set_geometry(NULL))
using.fips = split(using.fips[,c('STATE_FIPS','COUNTYFP')], f=1:NROW(using.fips))
...
BG = do.call(rbind, BG[!is.na(BG)]) %>% st_transform(st_crs(HOLC))

##--> Plot just CT
# 
BG %>%
  dplyr::filter(ST=='06') %>%
  ggplot(aes(fill = P080A001, color = P080A001)) +   #--> fill is fill, color is border (make same for no-border)
  geom_sf() +
...
BG = BG %>% dplyr::mutate(BG_AREA       = as.numeric(st_area(.)),
...
BG.int = st_intersection(BG, st_buffer(HOLC %>% group_by(GRADE) %>% dplyr::summarize(.), dist=0)) %>% 
          dplyr::mutate(shareArea = as.numeric(st_area(.)/BG_AREA),
                       Maj = shareArea>.80) %>%
          dplyr::arrange(GEOID, GRADE)


saveRDS(BG.int, file.path(WORK.OUT, paste0('BG_int----',Sys.Date(),'.rds')))
