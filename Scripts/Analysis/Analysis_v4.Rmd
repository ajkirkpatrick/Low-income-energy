...
```{r SubstandardOutput, echo=FALSE, fig.width=7, result='as.is', message=F, warning=F, cache = TRUE}
    ...
    my.femap = list( c('Control for home repair status 1935', rep('', 1), rep('X', 8)),
                    c('City-specific slope on Med. Income 2018, 1936', rep('X', length(BG.list2))),
                    c('City-specific slope on Mean Rent 1935', '',rep('X', 3), '', rep('X', 4)),
                    c('City-specific binary on Presence of Blacks 1935', rep('',2 ), rep('X', 7)),
                    c('Intx Predom. race 2018 and Presence of Blacks 1935', rep('', 6), rep('X', 3)) )
    ...
    Substandard.Output = modelsummary(models = BG.list2[model.use.vector], se = 'cluster',
                 ...
                 notes = list('Robust SE clustered by FIPS county','Omitted Grade: C (Yellow)')) %>%  # output as kableExtra
...
Response.Output.g1 = modelsummary(list(
             ...
             add_rows = as.data.frame(rbind(c('City-specific slopes for Sq. feet','X','X','X','X'),
...
# feols(u ~ hdd + hdd:GRADE.max + hdd:NITESET +  hdd:avginc  + hdd:Rent3739_Mean + hdd:NBlack_YN | IDENT, weights=~d, data = all09.use.pooled.e, warn = F), 
  ...
  add_rows = as.data.frame(rbind(c('Hh FE','X','X','X','X'),
                                 ...
                                 c('Controls for 1937 incl. rent, presence of Blacks','X','','','X'))),
