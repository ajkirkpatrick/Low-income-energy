...
BGZ = readRDS(jLoad(WORK.OUT, 'BG_ZCTA_int'))
BG.int.full = BGZ[[1]]
ZCTA.int.full = BGZ[[2]]
HOLC = readRDS(jLoad(WORK.OUT, 'HOLC_with_data')) # 10-12
...
summary(lm(OtherSubstandard ~ as.factor(GRADE.max) + as.factor(STCO) + MedIncome2018*Minc + Rent3739_Mean + bs(NBlack_PCT, 4), BG.int %>% dplyr::filter(share.max>.8)))
